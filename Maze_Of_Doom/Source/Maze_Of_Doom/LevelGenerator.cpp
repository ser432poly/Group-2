...
void ALevelGenerator::CreateLevel()
{
	...
	ARoom* currentRoom;
	int minPath = level % 5 + 5;
	int maxPath = level % 3 + 5;
	int roomLimit = 5 + level; //How many rooms to generate before all new rooms become deadends
	int chance;
	...
	while (!rooms.empty)
	{
		for (int i = 0; i < 4; i++)
		{
			int door = currentRoom->getDoor(i);
			if (door == 1)
			{
